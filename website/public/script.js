...
const reportUtils = window.RoadwatchReportUtils;
...
const configuredBaseUrl = (
  typeof window.ROADWATCH_PUBLIC_BASE_URL === "string"
  && window.ROADWATCH_PUBLIC_BASE_URL.trim()
)
  ? window.ROADWATCH_PUBLIC_BASE_URL.trim()
  : "";
...
function normalizeKey(key) {
  return reportUtils.normalizeKey(key);
}
...
function getFieldValue(record, aliases) {
  return reportUtils.getFieldValue(record, aliases);
}

function toFiniteCoordinate(value) {
  return reportUtils.toFiniteCoordinate(value);
}

function getCoordinatePair(report) {
  return reportUtils.getCoordinatePair(report);
}
...
function normalizeStatus(status) {
  return reportUtils.normalizeStatus(status);
}
...
function getIssueCategory(issueText, explicitCategory = "") {
  return reportUtils.getIssueCategory(issueText, explicitCategory);
}
...
function parseDateValue(rawValue) {
  return reportUtils.parseDateValue(rawValue);
}

function getSubmissionDate(report) {
  return reportUtils.getSubmissionDate(report);
}

function getResolutionDate(report) {
  return reportUtils.getResolutionDate(report);
}
...
function formatAverageResolution(hours) {
  return reportUtils.formatAverageResolution(hours);
}
